import sys,json,math,random,datetime,importlib.util
sys.path.insert(0,'scripts'); sys.path.insert(0,'/tmp')
spec=importlib.util.spec_from_file_location('rf','scripts/run_feb_to_june.py')
...
run_month=rf.run_month; tstat=rf.tstat
...
def ctl(panel,y,m,lev,cap,runs=200,seed=7):
    ...
    def sim(sides):
        ...
        for (ek,_),side in zip(ev,sides):
            ...
            if eq<=0: return -100.
        return (eq/1e5-1)*100
    act=sim([s for _,s in ev]); rng=random.Random(seed)
    outs=sorted(sim([rng.choice((-1,1)) for _ in ev]) for _ in range(runs))
    return dict(n=len(ev),actual_ret=act,random_beats=sum(1 for o in outs if o>act),
                runs=runs,random_median=outs[runs//2])

MON=[(2025,12,'DECEMBER2025'),(2026,1,'JANUARY2026'),(2026,2,'FEBRUARY2026'),(2026,3,'MARCH2026'),
     (2026,4,'APRIL2026'),(2026,5,'MAY2026'),(2026,6,'JUNE2026')]
res={}
for y,m,lab in MON:
    r=run_month(y,m,lab)
...
def pooled(panel):
    ks=sorted(panel)
    lo=datetime.datetime(2025,12,1,tzinfo=datetime.timezone.utc).timestamp()
...
res['POOLED_DEC_TO_JUNE']={'4H':pooled(p4h),'1H':pooled(p1h)}
...
short={'DECEMBER2025':'Dec','JANUARY2026':'Jan','FEBRUARY2026':'Feb','MARCH2026':'Mar','APRIL2026':'Apr','MAY2026':'May','JUNE2026':'Jun'}
ts={};ps={}
for _,_,lab in MON:
...
res['method']={
 ...
 'data':'100% Investing.com tvc4; ALL December windows envelope_fixes=0; 4H resampled from real 1H (00/04/08/12/16/20 UTC)',
 'controls':'random-direction control: same bars where alignment fires, coin-flip side, 200 runs, seed 7',
 'christmas_note':('Dec 25 2025 is a genuine market closure verified in the raw 1H feed: gold halts Dec 24 18:00 UTC and '
   'resumes Dec 25 23:00 UTC; EURUSD/AUDUSD halt Dec 24 21:00 UTC and resume Dec 25 22:00 UTC. The daily panel carries a '
   'Dec 25 bar, but the real intraday data shows only 1-2 late-evening bars that day, so it is a holiday stub, not a full session.')}
json.dump(res,open('backtest_results/live_model/dec2025_to_june2026_live_all_tf.json','w'),indent=1)

d=res['DECEMBER2025']
print('DEC coverage',d['coverage'])
for tf in ('DAILY','4H','1H'):
    b=d[tf]
    print(f"  {tf:5s} V1 n={b['V1_500x']['n']:4d} wr={b['V1_500x']['wr']:5.1f}% ret={b['V1_500x']['ret']:14.2f}% dd={b['V1_500x']['dd']:8.2f}% | V3 {b['V3_400x_cap2']['ret']:14.2f}% dd={b['V3_400x_cap2']['dd']:7.2f}% | t={b['raw_edge_1x']['t']:+.2f}")
print('\nDEC CONTROLS:')
for kk,v in d['random_direction_control'].items():
    print(f"  {kk:18s} act={v['actual_ret']:14.2f}% med={v['random_median']:14.2f}% beat={v['random_beats']}/200")
print('\nVALIDATION (must match published):')
for lab,exp in (('JANUARY2026','D +733.97 4H +353.22 1H -98.22'),('FEBRUARY2026','D -97.90 1H -99.88')):
    r=res[lab]
    print(f"  {lab}: D {r['DAILY']['V1_500x']['ret']:.2f}% 4H {r['4H']['V1_500x']['ret']:.2f}% 1H {r['1H']['V1_500x']['ret']:.2f}%   (expect {exp})")
P=res['POOLED_DEC_TO_JUNE']
for tf in ('4H','1H'):
    r=P[tf]; print(f"POOLED {tf}: n={r['raw_edge_1x']['n']} t={r['raw_edge_1x']['t']:+.2f} mean={r['raw_edge_1x']['mean']:+.4f}% | V1 {r['V1_500x']['ret']:.2f}% dd {r['V1_500x']['dd']:.2f}%")
