...
MON=[(2026,1,'JANUARY2026'),(2026,2,'FEBRUARY2026'),(2026,3,'MARCH2026'),(2026,4,'APRIL2026'),(2026,5,'MAY2026'),(2026,6,'JUNE2026')]
res={}
for y,m,lab in MON:
    r=run_month(y,m,lab)
...
res['POOLED_JAN_TO_JUNE']={'4H':pooled(p4h),'1H':pooled(p1h)}
...
short={'JANUARY2026':'Jan','FEBRUARY2026':'Feb','MARCH2026':'Mar','APRIL2026':'Apr','MAY2026':'May','JUNE2026':'Jun'}
ts={};ps={}
for _,_,lab in MON:
...
res['multiple_testing']={'tests':k,'bonferroni_alpha':alpha,'t_stats':ts,
 ...
 'note':'One p<0.05 out of 18 tests is what pure noise produces (expected 0.90). May 4H fails Bonferroni at alpha=0.002778.'}
res['method']={
 ...
 'data':'100% Investing.com tvc4; ALL January windows envelope_fixes=0; 4H resampled from real 1H (00/04/08/12/16/20 UTC)',
 'controls':'random-direction control: same bars where alignment fires, coin-flip side, 200 runs, seed 7',
 'validation':('January re-fetch reproduced all 507 overlapping bars from the previously stored Jan 1-14 data with ZERO mismatches; '
   'engine reproduces previously published Feb/Mar figures exactly. Superseded partial files data/raw/intraday/*2026w0*.json removed (fully contained in new jan windows).')}
json.dump(res,open('backtest_results/live_model/jan_to_june_2026_live_all_tf.json','w'),indent=1)

j=res['JANUARY2026']
print('JAN coverage',j['coverage'])
for tf in ('DAILY','4H','1H'):
    b=j[tf]
...
print('\nJAN CONTROLS:')
for kk,v in j['random_direction_control'].items():
    print(f"  {kk:18s} act={v['actual_ret']:14.2f}% med={v['random_median']:14.2f}% beat={v['random_beats']}/200")
print('\nFEB check (should match published): ',end='')
f=res['FEBRUARY2026']
print(f"D {f['DAILY']['V1_500x']['ret']:.2f}% 4H {f['4H']['V1_500x']['ret']:.2f}% 1H {f['1H']['V1_500x']['ret']:.2f}%")
P=res['POOLED_JAN_TO_JUNE']
for tf in ('4H','1H'):
